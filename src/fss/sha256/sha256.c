...
}; //as they are published by NIST

void sha256_init(SHA256_CTX* ctx){ //sqrts of first 8 primes - "Nothing-up-my-sleeve" 
...
static void sha256_compress(SHA256_CTX* ctx){ 
    uint32_t m[64];
    for (int i = 0; i < 16; i++) {
        m[i] = ((uint32_t)ctx->buffer[i*4]     << 24) |
...
void sha256_update(SHA256_CTX* ctx, const uint8_t* data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        ...
        ctx->buf_len++;
        if (ctx->buf_len == 64) {
            sha256_compress(ctx); //compress when 64 is reached
            ctx->buf_len = 0;
            ctx->bit_len += 512;
        }
    }
...
void sha256_final(SHA256_CTX* ctx, uint8_t digest[32]){
    uint64_t real_bytes = ctx->buf_len;
    ...
    int count = 0;
    for(int i = 63; i >= 56; i--){
        ctx->buffer[i] = (ctx->bit_len >> (count * 8)) & 0xFF;
        count++;
    }
